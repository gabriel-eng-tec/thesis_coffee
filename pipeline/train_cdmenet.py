...
Edición: Ajustes de warm-up, freeze/unfreeze, y logging
...
def train_epoch(model, dataloader, optimizer, epoch, total_epochs):
    ...
    use_aux = epoch >= EPOCH_WARMUP
    #w_class = 0.005 if use_aux else 0.0
    w_class = WEIGHTS['class_loss'] if use_aux else 0.0
    w_me    = WEIGHTS['mutual_exc'] if use_aux else 0.0
    w_semi  = WEIGHTS['semi_supervised'] if use_aux else 0.0
    ...
    for bidx, (imgs, dens) in enumerate(progress_bar):
        ...
        if has_label:
            ...
            # Corrección para PRESERVAR EL CONTEO al cambiar de resolución
            Hin, Win = dens.shape[-2], dens.shape[-1]
            Hout, Wout = density_pred.shape[-2], density_pred.shape[-1]
            if (Hout * Wout) > 0:
                dens_resized = dens_resized * (Hin * Win) / (Hout * Wout)
            ...
            #          WEIGHTS["class_loss"] * (loss_aux2 + loss_aux3 + loss_aux4)/3.0)
            loss = (WEIGHTS["density_loss"] * loss_density) + (w_class * loss_cls_avg) + loss_count
        else:
            # --- Pérdidas semi-supervisadas --- (apagado en warm-up)
            if w_semi > 0.0:
                ...
                loss += w_semi * (loss2 + loss3 + loss4) / 3.0
...
def main():
    ...
    # Modelo y optimizer
    model = CDMENet(image_size=IMAGE_SIZE).to(DEVICE)
    ...
    while start_epoch <= (EPOCHS_TOTAL + 1):
        ...
        for epoch in range(start_epoch, end_epoch + 1):
            ...
            # Descongelar frontend justo antes de activar auxiliares y bajar LR ×0.5
            if epoch == (EPOCH_WARMUP - 1):
            ...
            print(f"[{epoch:03d}/{EPOCHS_TOTAL}] "
                    f"Train Loss: {train_loss:.4f} | "
                    f"Val Loss: {val_loss:.4f} | "
                    f"Tiempo: {elapsed:.2f} min")
        ...
        # Salir tras este bloque (intencional para correr en tandas / reanudar con checkpoint)
        break
